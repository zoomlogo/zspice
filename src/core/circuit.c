...
circuit_t *new_circuit(void)
{
	...
	circuit->components =
	    (component_t *) calloc(circuit->component_capacity,
				   sizeof(component_t));
...
error_e c_add_connection(circuit_t *circuit, const component_t *component)
{
	...
	if (higher_id >= circuit->node_count) {
		...
		log_debug("resizing circuit->nodes with new capacity=%zu",
			  new_ncount);
		node_t *new_nodes =
		    (node_t *) realloc(circuit->nodes,
				       new_ncount * sizeof(node_t));
		if (new_nodes == NULL)
	...
	if (circuit->component_count >= circuit->component_capacity) {
		...
		log_debug("resizing circuit->components with new capacity=%zu",
			  new_ccap);
		component_t *new_components =
		    (component_t *) realloc(circuit->components,
					    new_ccap * sizeof(component_t));
...
error_e c_calculate_dim(circuit_t *circuit)
{
	...
	for (usize i = 0; i < circuit->component_count; i++) {
		...
		if (type == VOLTAGE_SOURCE
		    || type == INDUCTOR || type == VCVS || type == CCVS)
			circuit->components[i].solver_id =
			    circuit->node_count - 1 + unknwns++;
	}
	...
	log_info("circuit (%p) initialized with matrix dimensions=%zu", circuit,
		 circuit->dim);
	return OK;
}
