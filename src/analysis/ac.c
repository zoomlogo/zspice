...
error_e ac_sweep(circuit_t *circuit, ac_sweep_params_t *params, env_t *env)
{
	...
	if (env == NULL)
		env = &circuit->default_env;
	sbuf_t buffer;
	b_init(circuit->dim, true, &buffer);
	...
	case SWEEP_LINEAR:
		...
		multiplier = 1;
		step_size =
		    (params->stop_frequency -
		     params->start_frequency) / (params->steps - 1);
		break;
	case SWEEP_DECADE:
		net_steps =
		    params->steps * log10(params->stop_frequency /
					  params->start_frequency) + 1;
		multiplier = pow(10., 1. / (double)params->steps);
		step_size = 0;
		break;
	case SWEEP_OCTAVE:
		net_steps =
		    params->steps * log2(params->stop_frequency /
					 params->start_frequency) + 1;
	...
	for (usize i = 0; i < net_steps; i++) {
		...
		for (usize k = 0; k < params->n; k++) {
			...
			if (ref_id == 0)
				...
				ref_V = circuit->nodes[ref_id].zpotential;
		...
		// clamp
		if (i == net_steps - 2)
			current_freq = params->stop_frequency;
	}
