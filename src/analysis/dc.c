...
error_e dc_sweep(circuit_t *circuit, dc_sweep_params_t *params, env_t *env)
{
	...
	case SWEEP_LINEAR:
		...
		step_size =
		    (params->stop_voltage -
		     params->start_voltage) / (params->steps - 1);
		break;
	case SWEEP_DECADE:
		net_steps =
		    params->steps * log10(params->stop_voltage /
					  params->start_voltage) + 1;
		multiplier = pow(10., 1. / (double)params->steps);
		step_size = 0;
		break;
	case SWEEP_OCTAVE:
		net_steps =
		    params->steps * log2(params->stop_voltage /
					 params->start_voltage) + 1;
	...
	for (usize i = 0; i < net_steps; i++) {
		circuit->components[params->sweeped_component_id].V.dc_offset =
		    current_voltage;
		...
		// clamp
		if (i == net_steps - 2)
			current_voltage = params->stop_voltage;
	}
