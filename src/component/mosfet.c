...
error_e mosfet_linearize(component_t *c, env_t *env)
{
	...
	if (Vbs <= 0) {
		Vth =
		    VT0 - c->M.gamma * sqrt(2 * c->M.phi) +
		    c->M.gamma * sqrt(2 * c->M.phi - Vbs);
	...
	if (Vgs < Vth) {
		...
		Ids =
		    K * ((Vgs - Vth) * Vds - Vds * Vds / 2) * (1 +
							       c->M.lambda *
							       fabs(Vds));
		g_m = K * Vds * (1 + c->M.lambda * fabs(Vds));
		g_ds =
		    K * ((Vgs - Vth) * Vds -
			 Vds * Vds / 2) * c->M.lambda * sVds + K * (Vgs - Vth -
								    Vds) * (1 +
									    c->
									    M.
									    lambda
...
error_e dc_stamp_mosfet(sbuf_t *buf, component_t *c, env_t *env)
{
	...
	f64 Ieq =
	    c->M.Id - (c->M.g_m * c->M.Vgs + c->M.g_ds * c->M.Vds +
		       c->M.g_mb * c->M.Vbs);
	...
	if (nd > 0) {
		...
		if (ng > 0)
			A(nd - 1, ng - 1) += c->M.g_m;
		if (ns > 0)
			A(nd - 1, ns - 1) -= (c->M.g_ds + c->M.g_m + c->M.g_mb);
		if (nb > 0)
			A(nd - 1, nb - 1) += c->M.g_mb;
	}

	if (ns > 0) {
		if (nd > 0)
			A(ns - 1, nd - 1) -= c->M.g_ds;
		if (ng > 0)
			A(ns - 1, ng - 1) -= c->M.g_m;
		A(ns - 1, ns - 1) += (c->M.g_ds + c->M.g_m + c->M.g_mb);
		if (nb > 0)
			A(ns - 1, nb - 1) -= c->M.g_mb;
	}
