...
error_e dc_stamp_bjt(sbuf_t *buf, component_t *c, env_t *env)
{
	...
	if (nb > 0) {
		...
		if (nc > 0)
			A(nb - 1, ne - 1) += -c->Q.g_mu;
		if (ne > 0)
			A(nb - 1, ne - 1) += -c->Q.g_pi;
	}
	if (nc > 0) {
		if (nb > 0)
			A(nc - 1, nb - 1) += c->Q.g_mf - c->Q.g_mr - c->Q.g_mu;
		A(nc - 1, nc - 1) += c->Q.g_mr + c->Q.g_mu + c->Q.g_o;
		if (ne > 0)
			A(nc - 1, ne - 1) += -c->Q.g_mf;
	}
	if (ne > 0) {
		if (nb > 0)
			A(ne - 1, nb - 1) += c->Q.g_mr - c->Q.g_mf - c->Q.g_pi;
		if (nc > 0)
			A(ne - 1, nc - 1) += -c->Q.g_mr - c->Q.g_o;
	...
	if (nb > 0)
		buf->b[nb - 1] += lIb - c->Q.Ib;
	if (ne > 0)
		buf->b[ne - 1] += lIe - c->Q.Ie;
	if (nc > 0)
		buf->b[nc - 1] += lIc - c->Q.Ic;
